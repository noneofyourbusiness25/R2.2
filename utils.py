...
def extract_languages(text):
    ...
    for lang, tokens in LANGUAGES.items():
        ...
        pattern = r'(?:^|[^a-zA-Z0-9])(' + '|'.join(map(re.escape, tokens)) + r')(?:$|[^a-zA-Z0-9])'
    ...
    if REQUEST_TO_JOIN_MODE == True and join_db().isActive():
            ...
            if user and user["user_id"] == query.from_user.id:
                ...
                try:
                    ...
                    if user_data.status != enums.ChatMemberStatus.BANNED:
                        user_data2 = await bot.get_chat_member(UPDATES_CHANNEL, query.from_user.id)
                        if user_data2.status != enums.ChatMemberStatus.BANNED:
                            return True
                except UserNotParticipant:
                    pass
                except Exception as e:
        ...
        try:
            ...
            if user.status != enums.ChatMemberStatus.BANNED:
                user2 = await bot.get_chat_member(UPDATES_CHANNEL, query.from_user.id)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=usr_agent) as response:
                ...
                # Try to find the "Did you mean" suggestion, which is more reliable.
                suggestion_tag = soup.find('a', id='fprsl')
                if suggestion_tag:
                    return [suggestion_tag.text]

                # Fallback to scraping result titles if no suggestion is found
...
async def verify_user(bot, userid, token):
    ...
    if not await db.is_user_exist(user.id):
        await db.add_user(user.id, user.first_name)
        await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(user.id, user.mention))
    await db.update_verification(user.id, verified=True, verified_time=datetime.now())
    await db.update_token(user.id, token=None)
...
async def check_verification(bot, userid):
    ...
    if not await db.is_user_exist(user.id):
        await db.add_user(user.id, user.first_name)
        await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(user.id, user.mention))
    return await db.get_verification_status(user.id)
