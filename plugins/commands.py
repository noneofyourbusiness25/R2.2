...
from info import CLONE_MODE, OWNER_LNK, REACTIONS, CHANNELS, REQUEST_TO_JOIN_MODE, TRY_AGAIN_BTN, ADMINS, PREMIUM_AND_REFERAL_MODE, STREAM_MODE, AUTH_CHANNEL, REFERAL_PREMEIUM_TIME, REFERAL_COUNT, PAYMENT_TEXT, PAYMENT_QR, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT, MAX_B_TN, TUTORIAL, VERIFY_TUTORIAL, IS_TUTORIAL, URL
from utils import get_settings, pub_is_subscribed, get_size, is_subscribed, save_group_settings, temp, verify_user, check_token, check_verification, get_token, get_shortlink, get_tutorial, get_seconds, parse_info, format_caption
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        try:
            btn = [[
                    ...
                    InlineKeyboardButton("File Updates", url="https://t.me/Guide_for_Telegram_Files")
                ]]
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and (await db.get_bot_settings()).get('verify') == True:
                    btn = [[
            ...
            if STREAM_MODE == True:
                ...
                reply_markup = None
            msg = await send_media_with_flood_protection(
                client,
                chat_id=message.from_user.id,
                file_id=file_id,
                caption=f_caption,
...
async def send_media_with_flood_protection(client, chat_id, file_id, caption, protect_content, reply_markup):
    while True:
        try:
            msg = await client.send_cached_media(
                chat_id=chat_id,
